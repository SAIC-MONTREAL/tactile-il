...
plot_dir = "/user/t.ablett/datasets/contact-il/videos/sts_vs_panda_force/"
...
for task_i, (task, task_data) in enumerate(comp_data.TOY_TASK_DATA_DICT.items()):
    ...
    for ts in range(len(rel_poses)):

        ax = axd['traj']

        # traj_line = ax.plot(rel_poss[:, 2], rel_poss[:, 1], lw=line_width)[0]
        if ts == 0:
            traj_line = ax.plot(rel_poss[ts, 2], rel_poss[ts, 1], lw=line_width)[0]
        else:
            traj_line.set_xdata(rel_poss[:ts+1, 2])
            traj_line.set_ydata(rel_poss[:ts+1, 1])
        ...
        ax.set_xlim(task_data['live_traj_xlim'])
        ax.set_ylim(task_data['live_traj_ylim'])

        # midpath arrows
        if ts in task_data['traj_arrow_inds']:
            add_arrow(traj_line, xind=ts-1)

        # timestep labels
        x, y = rel_poss[:, 2], rel_poss[:, 1]
        # for ei, i in enumerate(task_data['traj_ts_labels']):
        if ts in task_data['traj_ts_labels']:
            i = ts
        ...
        ax.quiver(x_vals[:ts+1], y_vals[:ts+1], panda_forces[:ts+1, 2], panda_forces[:ts+1, 1], scale=scale, scale_units='height',
                width=shaft_width, headwidth=hs * 3.0, headlength=hs * 5.0, headaxislength=hs * 4.5, label='Panda',
                color='b')
        ...
        ax.quiver(x_vals[:ts+1], y_vals[:ts+1], sts_forces_rot[:ts+1, 2], sts_forces_rot[:ts+1, 1], scale=scale, scale_units='height',
                width=shaft_width, headwidth=hs * 3.0, headlength=hs * 5.0, headaxislength=hs * 4.5, label='STS',
                color='r')
        ...
        if task_data['contact_ts'] is not None:
            for ax_i, ax in enumerate([axd['panda_force'], axd['sts_force']]):
                ...
                if ax_i == 1:
                    ...
                    if ts >= li[0]:
                        for sub_ax_i, sub_ax in enumerate([axd['panda_force'], axd['sts_force']]):
                            ylim = sub_ax.get_ylim()
                            sub_ax.vlines(np.array(task_data['contact_ts'])[0] - .5, ylim[0] - .3, ylim[1] + .3, colors=['k'],
                                linestyle='--')
                    ...
                    if ts >= li[1]:
                        for sub_ax_i, sub_ax in enumerate([axd['panda_force'], axd['sts_force']]):
                            ylim = sub_ax.get_ylim()
                            sub_ax.vlines(np.array(task_data['contact_ts'])[1] - .5, ylim[0] - .3, ylim[1] + .3, colors=['k'],
                                linestyle='--')
        ...
        # fig.savefig(os.path.join(plot_dir, f'toy-{task_data["ds_substr"]}-{ts}.pdf'), bbox_inches='tight')
        fig.savefig(os.path.join(plot_dir, f'toy-{task_data["ds_substr"]}-{ts}.png'), bbox_inches='tight', dpi=300)

        print(f"Fig {task}, step {ts}")